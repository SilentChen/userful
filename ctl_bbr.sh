...
show_bbr_status()
{
		...
		echo " Arch           : $arch ($lbit Bit)"
		echo " Kernel         : $kern"
		echo " Lsmod grep bbr : `lsmod | grep bbr`"
	  echo " Sysctl net ipv4: `sysctl net.ipv4.tcp_available_congestion_control`"
		echo "----------------------------------------"
		echo " Auto install latest kernel for TCP BBR"
		echo "----------------------------------------"
		echo
...
undo_bbr()
{
	...
	fq_state="`cat ${path} | sed -n 's/#net.core.default_qdisc = fq/1/p'`"
	if [ $fq_state ]; then
		echo "--------------- The bbr already undo ---------------"
	else
		sed -ie 's/\bnet.core.default_qdisc = fq/#net.core.default_qdisc = fq/g' $path 
		sed -ie 's/\bnet.ipv4.tcp_congestion_control = bbr/#net.ipv4.tcp_congestion_control = bbr/g' $path
		echo "----- Donot forget to reboot, have a nice day.  ------"
	fi
	echo "------------------ Undo finish.  ---------------------"
}
...
redo_bbr()
{
	...
	echo "------------------ Redo begin:  ---------------------"
	path=/etc/sysctl.conf
	sed -ie 's/#net.core.default_qdisc = fq/net.core.default_qdisc = fq/g' $path 
	sed -ie 's/#net.ipv4.tcp_congestion_control = bbr/net.ipv4.tcp_congestion_control = bbr/g' $path
	echo "----- Donot forget to reboot, have a nice day.  ------"
	echo "------------------ Redo finish.  ---------------------"
}
...
Useage:
 ...
 |-redo  : Redo bbr.
 |-
 |-status: Show bbr relative status. 
EOF
}
